...
type Config struct {
	...
	DatabaseURL           string `env:"MUSIC_DATABASE_URL" envDefault:"postgres://postgres:postgres@host.docker.internal:5432/postgres?sslmode=disable"`
...
func (c *Config) GetConfig() error {
	...
	if _, check := os.LookupEnv("MUSIC_DATABASE_URL"); !check {
		return errors.New("environment variable MUSIC_DATABASE_URL is not set")
	}
...
// getDatabaseConnStr returns postgres connection string
func (c Config) GetDatabaseConnURL() string {
